...
#############################          Rishabh          ##################################
        ...
        #     for c in range(0, crop_image1.shape[1]-windowsize_c, windowsize_c):
            ...
            # window = cv2.cvtColor(, cv2.COLOR_BGR2GRAY)
            ...
            if count ==0:
                t=np.array(lbp)
                print "********************* initial t", t
                print "*********************initial shape t************",t.shape
                np.savetxt('initial_t.txt',t)
                count+=1
            else:
                t = np.concatenate((t,lbp))
                print "===========t========== final t",t
                print "======================t.shape final===========",t.shape
...
# training 
x1 = sigma1 * T1 + mean1
print "=========== X1 ===========", x1.shape, x1.size 
x2 = sigma2 * T2 + mean2
print "***********************************************"
print "=========== X2 ===========", x2.shape, x2.size 

reshape_value = (X_test.shape[0]*X_test.shape[1])/2

x1 = x1.reshape(Reshape_T1,2)
print x1
x2 = x2.reshape(Reshape_T2,2)
...
# prediction
X_test = X_test.reshape(reshape_value,2)
...
clf.probability
print "****************predict value*************",pred.all()
np.savetxt("predict_value", pred.all())
